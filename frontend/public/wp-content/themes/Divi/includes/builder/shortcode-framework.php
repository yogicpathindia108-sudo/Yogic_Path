...
function et_builder_load_modules_styles() {
	...
	if ( is_et_pb_preview() ) {
		...
		$preview_script_path = ET_BUILDER_DIR . 'frontend-builder/build/frontend-builder-preview.js';
		$preview_script_uri  = ET_BUILDER_URI . '/frontend-builder/build/frontend-builder-preview.js';

		// This file is produced by the frontend-builder minimal/full webpack flows. In clean zip/package runs,
		// avoid enqueuing a stale/broken script URL if the artifact is missing.
		if ( file_exists( $preview_script_path ) ) {
			wp_enqueue_script( 'et-builder-preview-script', $preview_script_uri, array( 'jquery' ), ET_BUILDER_VERSION, true );
			...
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- ET_DEBUG instrumentation.
			error_log(
				esc_html__( 'Divi preview requested, but frontend-builder preview runtime artifact is missing: frontend-builder/build/frontend-builder-preview.js.', 'et_builder' )
			);
		}
	}
}
...
function et_bfb_make_cpt_rewrite_rule_queryable( $value ) {
	...
	$has_post_type_substr = $rewrite_redirect && str_contains( $rewrite_redirect, '?post_type=' );
