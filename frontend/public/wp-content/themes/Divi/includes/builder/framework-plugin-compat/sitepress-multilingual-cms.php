...
class ET_Builder_Plugin_Compat_WPML_Multilingual_CMS extends ET_Builder_Plugin_Compat_Base {
	...
	function init_hooks() {
		...
		// Filter to check if the post is built with shortcodes.
		add_filter( 'wpml_pb_is_post_built_with_shortcodes', array( $this, 'maybe_post_build_with_d5' ), 11, 2 );

		// Exclude WPML filters when Divi 5 readiness.
		add_action( 'pre_get_posts', array( $this, 'maybe_exclude_wpml_filters' ) );
	}
	 ...
	 * Filter to check if the post is built with shortcodes.
	 *
	 * If the post is built with Divi 5, the filter will return false, indicating that the post is not built with shortcodes.
	 *
	 * @param bool    $is_built_with_shortcodes Indicates if the post is built with shortcodes.
	 * @param WP_Post $post The post object.
	 *
	 * @return bool false if the post is built with Divi 5, otherwise the original value of $is_built_with_shortcodes.
	 */
	public function maybe_post_build_with_d5( $is_built_with_shortcodes, $post ) {
		$et_pb_use_divi_5 = get_post_meta( $post->ID, '_et_pb_use_divi_5', true );

		if ( 'on' === $et_pb_use_divi_5 ) {
			return false;
		}

		return $is_built_with_shortcodes;
	}

	/**
	 * Remove WPML filters to ensure proper migration of translated posts to Divi 5.
	 *
	 * This function addresses the issue where translated posts are not fully migrated
	 * to Divi 5, resulting in compatibility warnings.
	 */
	public function maybe_exclude_wpml_filters() {
		global $wpml_query_filter;
		if ( et_core_security_check_passed( 'edit_posts', 'et_d5_readiness_get_result_list', 'wp_nonce' ) ) {
