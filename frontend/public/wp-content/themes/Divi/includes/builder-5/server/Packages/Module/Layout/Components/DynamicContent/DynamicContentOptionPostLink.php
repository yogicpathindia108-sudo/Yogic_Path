...
class DynamicContentOptionPostLink extends DynamicContentOptionBase implements DynamicContentOptionInterface {
	...
	public function register_option_callback( array $options, int $post_id, string $context ): array {
		$custom_fields = [
			'text'        => [
				'label'   => esc_html__( 'Link Text', 'et_builder_5' ),
				'type'    => 'select',
				'options' => [
					// Translators: %1$s: Post type name.
					'post_title' => esc_html( sprintf( __( '%1$s Title', 'et_builder_5' ), DynamicContentUtils::get_post_type_label( $post_id ) ) ),
					'custom'     => esc_html__( 'Custom', 'et_builder_5' ),
				],
				'default' => 'post_title',
			],
			'custom_text' => [
				'label'   => esc_html__( 'Custom Link Text', 'et_builder_5' ),
				'type'    => 'text',
				'default' => '',
				'show_if' => [
					'text' => 'custom',
