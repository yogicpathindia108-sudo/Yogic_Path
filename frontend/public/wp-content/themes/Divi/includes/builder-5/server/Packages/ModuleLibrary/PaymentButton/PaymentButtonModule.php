<?php
/**
 * Module Library: Payment Button Module
 *
 * @package Divi
 * @since ??
 */
 ...
 */
class PaymentButtonModule implements DependencyInterface {
	 ...
	 * ];
	 *
	 * PaymentButtonModule::module_classnames($args);
	 * ```
	 */
	public static function module_classnames( array $args ): void {
		...
		$classnames_instance->add( 'et_pb_payment_button' );
	...
	public static function wrapper_classnames( array $args ): void {
		...
		$classnames_instance->add( 'et_pb_empty_button', ! $text_value );
		$classnames_instance->add( 'et_pb_payment_button_module_wrapper' );
		$classnames_instance->add( "et_pb_payment_button_{$args['orderIndex']}{$wrapper_prefix}_wrapper" );
	}
	 ...
	 * );
	 *
	 * PaymentButtonModule::module_script_data( $args );
	 * ```
	 */
			...
			[
				...
				'setVisibility' => [
					[
						'data'          => $attrs['button']['innerContent'] ?? [],
						'valueResolver' => function ( $value ) {
							return empty( $value['text'] ) ? 'hidden' : 'visible';
						},
					],
				],
			]
	 ...
	 *
	 * PaymentButtonModule::render_callback( $attrs, $child_modules_content, $block, $elements, $default_printed_style_attrs );
	 * ```
	 */
	public static function render_callback( array $attrs, string $child_modules_content, WP_Block $block, ModuleElements $elements, array $default_printed_style_attrs ): string {
		...
		foreach ( $button_decoration as $breakpoint => $breakpoint_value ) {
			...
			if ( $is_icon_enabled && isset( $icon_settings ) ) {
				$processed_icon = IconFontUtils::process_font_icon( $icon_settings );
		...
		// Nothing to output if Button Text is not defined.
		if ( empty( $text_value ) ) {
			return '';
		}
		...
		// Resolve the deterministic checkout URL server-side and bake it into the markup,
		// mirroring how the Button module renders its link.
		$payment_value   = $attrs['button']['innerContent']['desktop']['value'] ?? [];
		$open_in_new_tab = 'on' === ( $payment_value['openInNewTab'] ?? 'off' );
		$link_target     = $open_in_new_tab ? '_blank' : null;

		$provider   = (string) ( $payment_value['provider'] ?? PaymentAccountService::PAYPAL_PROVIDER );
		$return_url = (string) ( $payment_value['returnUrl'] ?? '' );
		$cancel_url = (string) ( $payment_value['cancelUrl'] ?? '' );

		if ( PaymentAccountService::PAYPAL_PROVIDER === sanitize_key( $provider ) ) {
			$current_page_url = '';

			if ( '' === $return_url || '' === $cancel_url ) {
				$current_page_url = self::_get_current_page_url();
			}
		...
		$resolved_amount = StyleLibraryUtils::resolve_dynamic_variables_recursive( $payment_value['amount'] ?? '' );
		$amount_value    = is_scalar( $resolved_amount ) ? (string) $resolved_amount : '';

		$checkout_url = PaymentAccountService::create_checkout_url(
			$provider,
			(string) ( $payment_value['resourceId'] ?? '' ),
			(string) ( $payment_value['environment'] ?? 'sandbox' ),
			$amount_value,
			(string) ( $payment_value['currency'] ?? 'USD' ),
			(string) ( $payment_value['amountMode'] ?? 'fixed' ),
			$return_url,
			$cancel_url,
			(string) ( $payment_value['description'] ?? '' )
		);

		$link_value    = is_wp_error( $checkout_url ) ? '' : (string) $checkout_url;
		$validated_url = ! empty( $link_value ) ? esc_url( $link_value ) : '';

		$html_attrs = [
			'target' => ( ! empty( $link_target ) && ! empty( $validated_url ) ) ? esc_attr( $link_target ) : null,
			'rel'    => ( $open_in_new_tab && ! empty( $validated_url ) ) ? 'noopener noreferrer' : null,
		];
	...
	/**
	 * Get the current page URL where payment was initiated.
	 *
	 * @since ??
	 *
	 * @return string
	 */
	private static function _get_current_page_url(): string {
		$candidate_url = URLUtility::get_canonical_current_request_url();
	 ...
	 * ];
	 *
	 * PaymentButtonModule::position_width_style_declaration($params);
	 * ```
	 */
	...
	public static function module_styles( array $args ): void {
		$default_attributes          = ModuleRegistration::get_default_attrs( 'divi/payment-button' );
	...
	public static function icon_style_declaration( array $params ): string {
		...
		if ( ! empty( $settings ) ) {
			$icon_unicode = IconFontUtils::escape_font_icon( IconFontUtils::process_font_icon( $settings ) );
	...
	public function load(): void {
		$module_json_folder_path = dirname( __DIR__, 4 ) . '/visual-builder/packages/module-library/src/components/payment-button/';
